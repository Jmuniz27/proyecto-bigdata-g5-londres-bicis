...
Un paso de ETL ejecutado una sola vez por versión reduce el tiempo total entre $20\times$ y $57\times$ en Pandas, entre $3\times$ y $11\times$ en DuckDB y hasta $2\times$ en SQLite; en Pandas y DuckDB se amortiza antes de completar una sola corrida.

\subsection{Diagnóstico}
\label{sec:fase3-diagnostico}
...
\begin{enumerate}
  ...
  \item Se leen nueve columnas y solo se usan tres: \texttt{start\_rental\_date\_time}, \texttt{start\_station\_id} y \texttt{end\_station\_id}. Las dos columnas de nombres de estación son las más pesadas y en general son redundantes, porque ya están normalizadas en el catálogo. La excepción son las 7 estaciones sin entrada en \texttt{london\_stations.csv} (Sección \ref{sec:dataset}): para esas, la columna de nombre es la única fuente y su valor se pierde de forma irrecuperable al proyectar a 4 columnas.
