...
La propuesta inicial planteó siete preguntas exploratorias; se eligieron las tres primeras porque las tres se resuelven con un \texttt{GROUP BY} sobre las mismas columnas, lo que aísla el efecto de la herramienta. Las cuatro restantes (duración, geografía, período de fiestas y distancia/velocidad) quedaron fuera del alcance.
...
Cada combinación de versión, herramienta y pregunta es una \textbf{ejecución independiente que arranca desde el archivo}: se carga desde cero y después se corre la consulta. El tiempo reportado es carga $+$ query, y ese es el número que se compara contra el límite de 10 minutos. El límite se verifica al terminar cada ejecución: la que lo supera se marca \texttt{EXCEDE} y las preguntas siguientes de esa herramienta se omiten.

La razón es que SQLite no conserva el archivo cargado entre consultas, a diferencia de Pandas y DuckDB, que mantienen los datos en memoria. El mismo protocolo para las tres evita que alguna se beneficie de reutilizar una carga que otra tuvo que repetir.
