...
6 & \textbf{El import de \texttt{pyarrow} inflaba el tiempo de carga del Parquet.} Pandas lo importa en el primer \texttt{read\_parquet()} y ese import en frío se contaba como lectura de datos. &
Se movió el import fuera de todo bloque cronometrado. \\
7 & \textbf{Los artefactos pesados en carpetas sincronizadas distorsionaban los tiempos}, porque la sincronización compite por I/O. &
Los \texttt{.db} y \texttt{.parquet} se escriben en disco local, no en la carpeta de nube. \\
8 & \textbf{Pc\_Martha (15.7 GB) falló en XL tras 17 minutos} sin completar la lectura del CSV. &
Se registró como resultado: es el dato que define el umbral de Big Data para ese equipo. Se corrió una segunda pasada sin XL para completar las demás versiones. \\
9 & \textbf{La gráfica de impacto suma tiempos sin verificar que ambas etapas tengan las mismas preguntas completadas.} Con ejecuciones omitidas, la barra base queda corta y sugiere que la optimización empeoró el rendimiento. &
Se detectó en SQLite XL sobre Pc\_Nahin y se corrigió comparando pregunta contra pregunta (\S\ref{sec:nota-nahin}). \\
\end{longtable}
}
